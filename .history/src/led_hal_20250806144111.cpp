...
LedHAL_GPIO::LedHAL_GPIO(uint8_t ledPin) : PA_c(ledPin)
{
...
void LedHAL_GPIO::ConfigurateAsOutput()
{
    ...
    GPIOA->MODER    &= ~(BIT_MASK_2_BIT << (PA_c * 2));
    GPIOA->MODER    |= (GpioConfig::ModeOutput  << (PA_c * 2));

    // set otyper -> push/pull
    GPIOA->OTYPER   &= ~(BIT_MASK_1_BIT << PA_c);

    // reset ospeedr -> set ospeedr -> high speed
    GPIOA->OSPEEDR  &= ~(BIT_MASK_2_BIT << (PA_c * 2));
    GPIOA->OSPEEDR  |= (GpioConfig::SpeedHigh << (PA_c * 2));

    // reset pupdr -> set pupdr -> deactivate (no pull)
    GPIOA->PUPDR    &= ~(BIT_MASK_2_BIT << (PA_c * 2));
    GPIOA->PUPDR    |= (GpioConfig::NoPull << (PA_c * 2));
}
